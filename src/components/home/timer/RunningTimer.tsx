...
import { deleteTimer, setTimerSound, updateTimerLabel, updateTimerTime } from "@/redux";
...
function RunningTimer(props: RunningTimerProps) {
    ...
    const timerAudio = Array.from(
        { length: stateData.timer.timerSounds.length },
        useRef
    );
    ...
    useEffect(() => {
        ...
        setTimerRingtonesLength(stateData.timer.timerSounds.length);
    }, [stateData.timer]);
    ...
    const playTimerSound = (value: string) => {
        ...
        timerAudio[stateData.timer.timerSounds.indexOf(value)].current.play();
    };

    return (
        <>
            {stateData.timer.timerSounds.map((value: string, index: number) => {
                return (
                    <span key={index}>
                        <audio
                    ...
                    return (
                        <>
                            ...
                            <CustomDialog
                                ...
                                data={stateData.timer.timerSounds}
                                keepMounted
                                value={timer.sound}
